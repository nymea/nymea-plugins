...
void IntegrationPluginGaradget::setupThing(ThingSetupInfo *info)
{
    ...
    if (!(device.startsWith( "garadget/"))) {
        if (device.contains("/")) {
            return info->finish(Thing::ThingErrorInvalidParameter, QT_TR_NOOP(QString ("The given deviceName is not valid.")));
        }
